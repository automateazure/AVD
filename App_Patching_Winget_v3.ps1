...
Split-Path -Path "C:\Program Files\WindowsApps\Microsoft.DesktopAppInstaller_*_x64__8wekyb3d8bbwe" -Parent
cd $wingetpath


# Create $upgradeResult variable that references "winget upgrade" to list available software updates
$upgradeResult = winget upgrade --accept-source-agreements | Out-String
...
foreach ($package in $upgradeList) 
{
    if (-not ($toSkip -contains $package.Id)) 
    {
        ...
        & winget upgrade --exact --silent --accept-source-agreements --accept-package-agreements $package.id
    }
    else 
    {    
